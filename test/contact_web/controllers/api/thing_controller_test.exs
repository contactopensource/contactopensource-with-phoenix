defmodule ContactWeb.Api.ThingControllerTest do
  ...
  describe "index" do
    test "lists all things", %{conn: conn} do
      conn = get(conn, Routes.api_thing_path(conn, :index))
  ...
  describe "create thing" do
    test "renders thing when data is valid", %{conn: conn} do
      conn = post(conn, Routes.api_thing_path(conn, :create), thing: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.api_thing_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.api_thing_path(conn, :create), thing: @invalid_attrs)
  ...
  describe "update thing" do
    ...
    test "renders thing when data is valid", %{conn: conn, thing: %Thing{id: id} = thing} do
      conn = put(conn, Routes.api_thing_path(conn, :update, thing), thing: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, thing: thing} do
      conn = put(conn, Routes.api_thing_path(conn, :update, thing), thing: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete thing" do
    ...
    test "deletes chosen thing", %{conn: conn, thing: thing} do
      conn = delete(conn, Routes.api_thing_path(conn, :delete, thing))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.api_thing_path(conn, :show, thing))
      end
    end
  end
