defmodule ContactWeb.EdgeControllerTest do
  ...
  describe "new edge" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.edge_path(conn, :new))
  ...
  describe "create edge" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.edge_path(conn, :show, id)
  ...
  describe "edit edge" do
    ...
    test "renders form for editing chosen edge", %{conn: conn, edge: edge} do
      conn = get(conn, Routes.edge_path(conn, :edit, edge))
  ...
  describe "update edge" do
    ...
    test "redirects when data is valid", %{conn: conn, edge: edge} do
      ...
      assert redirected_to(conn) == Routes.edge_path(conn, :show, edge)

      conn = get(conn, Routes.edge_path(conn, :show, edge))
      assert html_response(conn, 200) =~ "some updated object_database"
    end
  ...
  describe "delete edge" do
    ...
    test "deletes chosen edge", %{conn: conn, edge: edge} do
      ...
      assert redirected_to(conn) == Routes.edge_path(conn, :index)
